...
export class DiskioFileSmart {
    ...
    public async write(data: Buffer): Promise<IDiskIOFileManifest> {
        ...
        const chunkPromises: Promise<IChunkManifest>[] = parts.map(async (part) => {
            // Hash the part
            ...
            if (exists) {
                ...
                // Return hash
                return { hash, original: part.length, size: size as number };
            }
            // Compress the data
            const compressed = await compress(part, 3);
            // Create file
            ...
            return { hash, original: part.length, size: compressed.length };
        });
